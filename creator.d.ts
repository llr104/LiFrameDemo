	...
	*/
	...
	export class Node extends _BaseNode {		
		...
		*/
		...
		please do not use cc.eventMgr directly for Node.
		!#zh
		在节点上注册指定类型的回调函数，也可以设置 target 用于绑定响应函数的 this 对象。<br/>
		同时您可以将事件派发到父节点或者通过调用 stopPropagation 拦截它。<br/>
		推荐使用这种方式来监听节点上的触摸或鼠标事件，请不要在节点上直接使用 cc.eventMgr。
	...
	<br>
	cc.eventMgr is a singleton object which manages event listener subscriptions and event dispatching.
	...
	export class eventMgr {		
		...
		*/
		...
		// 1. remove eventMgr add Listener;
		var mouseListener1 = cc.eventMgr.addListener({
		...
		cc.eventMgr.removeListener(mouseListener1);
		...
		cc.eventMgr.removeListener(mouseListener2);
		...
		*/
		...
		!#zh
		移除注册到 eventMgr 中指定类型的所有事件监听器。<br/>
